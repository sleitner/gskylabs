...
my $url = "http://casjobs.sdss.org/dr7/en/tools/search/x_sql.asp?format=csv&cmd=SELECT+TOP+500+p.ra,p.dec,p.type,p.modelmag_g,p.modelmag_r,p.modelmag_i,p.run,p.rerun,p.camcol,p.field,p.obj,p.objid,dbo.fIAUFromEq(p.ra,p.dec)+FROM+Star+as+p+WHERE+p.modelmag_r>0+and+p.modelmag_r<21+and+p.ra<$east+and+p.ra>$west+and+p.dec>$south+and+p.dec<$north+order+by+p.modelmag_r";
