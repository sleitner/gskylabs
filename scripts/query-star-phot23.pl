...
my $url = "http://casjobs.sdss.org/dr7/en/tools/search/x_sql.asp?format=csv&cmd=SELECT+TOP+500+p.ra,p.dec,p.type,p.modelmag_g,p.modelmag_r,p.modelmag_i,p.run,p.rerun,p.camcol,p.field,p.obj,p.objid,dbo.fIAUFromEq(p.ra,p.dec)+FROM+Star+as+p+WHERE+p.modelmag_r<23+and+p.ra<$east+and+p.ra>$west+and+p.dec>$south+and+p.dec<$north+order+by+p.modelmag_r";
#my $url = "http://casjobs.sdss.org/dr7/en/tools/search/x_sql.asp?format=csv&cmd=SELECT+TOP+500+p.ra,p.dec,p.type,p.modelmag_g,p.modelmag_r,p.modelmag_i,p.run,p.rerun,p.camcol,p.field,p.obj,p.objid,dbo.fIAUFromEq(p.ra,p.dec)+FROM+PhotoObj+as+p+WHERE+p.modelmag_r<21+and+p.ra<$east+and+p.ra>$west+and+p.dec>$south+and+p.dec<$north+order+by+p.modelmag_r";
#my $url = "http://casjobs.sdss.org/dr7/en/tools/search/x_sql.asp?format=csv&cmd=SELECT+TOP+500+p.ra,p.dec,p.type,p.modelmag_g,p.modelmag_r,p.modelmag_i,p.run,p.rerun,p.camcol,p.field,p.obj,p.objid,dbo.fIAUFromEq(p.ra,p.dec)+FROM+Star+as+p+WHERE+p.ra<$east+and+p.ra>$west+and+p.dec>$south+and+p.dec<$north+order+by+p.modelmag_r";
warn "$url";
my $content = get $url;
...
@lines = split "\n", $content;
if($#lines == 0){ error_kml("No Data in SDSS", "There is no data in DR5 for the viewport location", $px, $py); }
shift(@lines);
...
for $line (@lines){
  ($x, $y, $type, $g, $r, $i, $run, $rerun, $camcol, $field, $obj, $objid, $name) = split ',', $line;
  if($type == 0){
    $tstr = "UNKNOWN";
  }
  elsif($type == 1){
  ...
  printf "    <tr><td>MAGS:</td><td> g=%2.4f </td></tr><tr><td></td><td>r=%2.4f </td></tr><tr><td></td><td>i=%2.4f</td></tr><tr><td>Photometric Redshift:</td><td>$redz</td></tr><tr><td>Photo-Z Error:</td><td>$zerr</td></tr></table><a href=\"http://cas.sdss.org/astro/en/tools/explore/obj.asp?id=$objid\">Click for SDSS Page</a><br/>Note: All magnitudes are model mags.\n",$g,$r,$i;
  print "    ]]></description>\n";
